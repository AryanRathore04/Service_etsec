...
const GetStartedForm = ({
  ...
  useEffect(() => {
    // Derive current selectedGroup from state to avoid stale closures
    const selectedGroup = formData.selectedGroup;

    if (selectedGroup) {
      const group = serviceGroups.find((g) => g.id === selectedGroup);
      setCurrentServices(group ? group.services : []);
    } else {
      setCurrentServices([]);
    }

    // If initialServices were provided and the selectedGroup matches the initialGroup,
    // keep those selected services. Otherwise clear selectedServices when group changes.
    if (initialServices && initialServices.length > 0 && selectedGroup === initialGroup) {
      setFormData((prev) => ({ ...prev, selectedServices: initialServices }));
    } else {
      setFormData((prev) => ({ ...prev, selectedServices: [] }));
    }
    // Include initialGroup and initialServices in dependencies to satisfy exhaustive-deps
  }, [formData.selectedGroup, initialGroup, initialServices]);
  ...
  const handleSubmit = (e) => {
    ...
    if (
      ...
      alert(
        "Please fill in your contact details, select a service group, and at least one service."
      );
      return;
    }
    ...
    fetch("/", {
      ...
      .then(() => {
        alert(
          "Thank you! Your inquiry has been submitted. We will get back to you with a quote shortly."
        );
    ...
    <div className="popup-overlay" onClick={onClose}>
      <div className="popup-content" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="popup-subtitle">
          Tell us about your needs, and we'll send a custom quote to your email
          shortly.
        </p>

        <form
          name="getStartedContact"
          method="POST"
          data-netlify="true"
          onSubmit={handleSubmit}
          className="custom-getstarted-form"
        >
          ...
          <p className="pricing-disclaimer">
            * Exact pricing will be provided via email shortly after reviewing
            your requirements.
          </p>
